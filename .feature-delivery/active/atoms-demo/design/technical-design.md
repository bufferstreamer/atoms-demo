...
| 生成 | 无模型或生成服务 | 可解释的意图分类 + AppSpec v1 模板组合 + 版本化配置 | 不接入外部模型密钥 |
...
flowchart LR
  ...
  ORCH --> GEN["Deterministic AppSpec Generator"]
  API --> DB[("D1")]
  GEN --> VALIDATE["AppSpec Validator"]
  VALIDATE --> DB
...
Run 状态：`RUNNING -> COMPLETED | FAILED`；Step 状态：`PENDING -> RUNNING -> COMPLETED | FAILED`。固定四步为 `product`、`architecture`、`design`、`engineering`。服务端同步完成确定性生成并持久化全部 step；客户端收到快照后用 350ms 间隔依次揭示已保存 step，明确这是可读执行回放，不伪装为模型流式输出。
...
请求在最终 batch 前中断可能遗留 RUNNING。任何 workspace/project 读取都会用一个 D1 batch 把 `updated_at` 超过 2 分钟的 RUNNING run 标记为 FAILED、把关联 `workspace_requests`（如有）标记为 FAILED/`RUN_TIMEOUT`、把尚未完成的 step 标记为 FAILED，从而释放锁并保留上一成功版本。相同首次 requestId 重试返回该已保存超时错误；用户点击“重新生成”必须创建新的 requestId。读取失败时 UI 保留当前画面并提供重试，不清空输入或缓存快照。
...
- `runs(id, project_id, request_id, base_version_id, status, error_code, created_at, updated_at)`；唯一 `(project_id, request_id)` 与 partial unique RUNNING。
