...
| GAP-004 | IMPLEMENTATION | OPEN | CHG-006 尚未部署并证明 JSON Schema/对象响应路径的 Llama 在线成功 | 只有部署 CHG-006 后同一 commit/Worker version/bindings、同一 requestId/runId 的 `workers_ai/@cf/meta/llama-3.1-8b-instruct-fast/SUCCESS` 同时满足模型 `<55s`、API `<65s`，并写入 `artifacts/chg006-online-model-2026-08-09.md` 才能关闭；`adce530c`、CHG-005 artifact、GLM 或任意历史 event 均无效 |
| GAP-005 | ACCEPTED_RISK | ACCEPTED | Workers AI 免费日额度耗尽时生成会降级为规则引擎 | UI 明示生成来源；D1 审计区分成功与降级；不影响已有项目和版本读取 |

## 完成声明边界

首版已发布；CHG-006 当前只能声明结构化输出适配在实现中。在取得上述同次 Llama `workers_ai/SUCCESS` 证据前，不能声明真实模型已接通。
